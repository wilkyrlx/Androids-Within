...
class assassin implements IGameRules {
    ...
    assignRoles() {
        ...
        for (let i = 3; i < 3 + minorityCount; i++) {
            this.assignments[this.players[i]] = "You are an Informant. Secretly tell the Assassin " + this.players[0] + " that his target is " + this.players[1];
        }
        for (let i = 3 + minorityCount; i < 3 + minorityCount + majorityCount; i++) {
            this.assignments[this.players[i]] = "You are an Informant. Secretly tell the Assassin " + this.players[0] + " that his target is " + this.players[2];
        }   
    }
